<template>
  <section v-if="report" class="report-panel">
    <div class="report-panel__summary">
      <div>
        <p class="eyebrow">{{ t('report.totalScore') }}</p>
    ...
    <div v-if="report.usage" class="usage-panel">
      ...
      <details class="usage-records">
        <summary>{{ t('report.records') }}</summary>
        <article v-for="record in report.usage.records" :key="record.id" class="usage-record">
    ...
    <RadarChart :tests="report.tests" />

    <div class="result-list">
      <article v-for="test in report.tests" :key="test.id" class="result-row">
        <div>
...
import type { ProbeCallRecord, ProbeReport } from '../types/probe'
